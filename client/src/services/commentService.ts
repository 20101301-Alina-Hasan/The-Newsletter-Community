...
export const fetchComments = async (news_id: number) => {
    try {
        ...
        const response = await axios.get(`http://localhost:4000/api/comments/${news_id}`);
...
export const addComment = async (news_id: number, comment: string, token: string) => {
    try {
        ...
        const response = await axios.post(`http://localhost:4000/api/comments/${news_id}`, { comment }, {
            headers: {
...
export const editComment = async (comment_id: number, editedComment: string, token: string) => {
    try {
        ...
        const response = await axios.put(`http://localhost:4000/api/comments/${comment_id}`, commentData, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
            },
...
export const deleteComment = async (comment_id: number, token: string) => {
    try {
        ...
        const response = await axios.delete(`http://localhost:4000/api/comments/${comment_id}`, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
            },
